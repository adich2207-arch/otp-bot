...
# ── Database ──────────────────────────────────────────────────────────────────
def _db_url() -> str:
"""Return a psycopg-compatible connection URL with SSL forced on."""
url = DATABASE_URL or ""
...
def get_db():
"""Open a single direct psycopg connection, yield it, then close it.
Retries on transient SSL errors so the bot never crashes mid-request."""
last_err = None
...
last_err = e
logger.warning(f"DB connect attempt {attempt+1}/3 failed: {e}. Retrying...")
time.sleep(2)
raise last_err

def init_db():
"""Create tables. Retries up to 10 times for Render's cold-start DB delay."""
...
logger.warning(f"init_db attempt {attempt}/10 failed: {e}. Retrying in {wait}s...")
time.sleep(wait)
raise RuntimeError("❌ Could not connect to PostgreSQL after 10 attempts. Check DATABASE_URL.")
...
logger.warning(f"ensure_user({user_id}) failed: {e}")
...
logger.warning(f"get_balance({user_id}) failed: {e}")
return 0.0
...
logger.warning(f"get_referral_count({user_id}) failed: {e}")
return 0
...
logger.warning(f"get_referral_earnings({user_id}) failed: {e}")
return 0.0
...
f"🕐  <b>Submitted for admin review</b>"
)
...
f"╚══════════════════════╝\n\n"
f"🆔  <b>Ref ID:</b>    <code>#{dep_id}</code>\n"
f"👤  <b>User:</b>     <code>{dep['user_id']}</code>\n"
f"💵  <b>Amount:</b>   <b>{fmt(dep['amount'])}</b>\n"
f"📊  <b>Status:</b>   ✅ <i>Credited to Wallet</i>\n"
+ (f"🤝  <b>Referral:</b>  <b>{fmt(commission)}</b> paid\n" if commission else "") +
f"\n🤖  @OtpSellerStore_Bot",
...
f"👤  <b>User:</b>     <code>{dep['user_id']}</code>\n"
f"💵  <b>Amount:</b>   <b>{fmt(dep['amount'])}</b>\n"
f"📊  <b>Status:</b>   ❌ <i>Payment Not Verified</i>"
)
...
f"🌍  <b>Country:</b>  {flag} <b>{country_name}</b>\n"
f"📱  <b>Phone:</b>    <code>{mask_phone(phone_raw)}</code>\n"
f"💵  <b>Price:</b>    <b>{fmt(price)}</b>\n"
f"📦  <b>Stock:</b>    +1 unit\n"
f"📊  <b>Status:</b>   🟢 <i>Available Now</i>\n\n"
...
async def buy_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Show server selection: Server (1), Server (2), back."""
    query = update.callback_query
    await query.answer()
    await query.edit_message_text(
        "<b>🛒 BUY ACCOUNT</b>\n"
        "<b>▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰</b>\n\n"
        "Select a server to browse accounts:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
...
async def buy_server_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Show available countries as 2-column grid: 🏳 CC+dial | price$  with pagination."""
    query = update.callback_query
    await query.answer()

    # Support pagination via callback_data="buy_server_1" / "buy_server_2" or "buy_page_2" etc.
    page = 0
...
# ── Launch OTP watcher as a background task ──────────────────────────────
import asyncio
asyncio.create_task(
...
f"🆔  <b>Account:</b>  <code>#{acc_id}</code>\n"
f"🌍  <b>Country:</b>  {flag} <b>{country_name}</b>\n"
f"📱  <b>Phone:</b>    <code>{mask_phone(phone)}</code>\n"
f"💵  <b>Price:</b>    <b>{fmt(acc['price'])}</b>\n"
f"👤  <b>Buyer:</b>    <code>{user_id}</code>\n"
f"📊  <b>Status:</b>   ✅ <i>Delivered</i>\n\n"
f"🤖  @OtpSellerStore_Bot",
...
f"📱  <b>Phone:</b>    <code>{mask_phone(phone)}</code>\n"
f"👤  <b>Seller:</b>   {seller_name} (<code>{user.id}</code>)\n"
f"📊  <b>Status:</b>   🔄 <i>Pending Admin Review</i>"
)
...
f"💵  <b>Price:</b>    <b>{fmt(price)}</b>\n"
f"📊  <b>Status:</b>   🟢 <i>Live in Marketplace</i>"
)
return ConversationHandler.END
...
f"🆔  <b>Account:</b>  <code>#{acc_id}</code>\n"
f"🌍  <b>Country:</b>  {flag} <b>{country_name}</b>\n"
f"📊  <b>Status:</b>   ❌ <i>Removed from Queue</i>"
)
...
async def withdraw_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if balance < 2.0:
        min_inr = round(2.0 * USD_TO_INR)
        await query.edit_message_text(
            f"<b>💸 WITHDRAW</b>\n"
            f"<b>▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰</b>\n\n"
            f"<b>❌ Minimum Withdrawal Not Met</b>\n\n"
            f"<b>💰 Your Balance: {fmt(balance)}</b>\n"
            f"<b>📌 Minimum Required: ₹{min_inr} ($2.00)</b>\n\n"
            f"Please recharge your wallet to reach the minimum withdrawal amount.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💵  Recharge", callback_data="menu_deposit")],
                [InlineKeyboardButton("🔙  Back to Menu", callback_data="menu_back")],
...
if amount_usd < 2.0:
    ...
    await update.message.reply_text(
        ...
        f"The amount you entered (<b>₹{amount_inr:.0f}</b>) is below the minimum.\n\n"
...
f"╚══════════════════════╝\n\n"
f"🆔  <b>Ref ID:</b>    <code>#{wd_id}</code>\n"
f"👤  <b>User:</b>     <code>{user.id}</code>\n"
f"💵  <b>Amount:</b>   <b>{fmt(amount_usd)}</b>\n"
f"📊  <b>Status:</b>   ⏳ <i>Awaiting Processing</i>"
)
...
f"╚══════════════════════╝\n\n"
f"🆔  <b>Ref ID:</b>    <code>#{wd_id}</code>\n"
f"👤  <b>User:</b>     <code>{wd['user_id']}</code>\n"
f"💵  <b>Amount:</b>   <b>{fmt(wd['amount'])}</b>\n"
f"📊  <b>Status:</b>   ✅ <i>Payment Processed</i>\n\n"
f"🤖  @OtpSellerStore_Bot"
)
...
f"👤  <b>User:</b>     <code>{wd['user_id']}</code>\n"
f"💵  <b>Amount:</b>   <b>{fmt(wd['amount'])}</b>\n"
f"📊  <b>Status:</b>   ❌ <i>Rejected — Funds Returned</i>"
)
...
f"🏦 <b>Wallet Funds:</b>       <b>{fmt(float(total_balance))}</b>\n"
f"📦 <b>Available Accounts:</b> <b>{avail_accounts}</b>\n\n"
f"<b>▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰</b>\n"
f"📥 <b>Pending Deposits:</b>    <b>{pending_deps}</b>  {'🔴' if pending_deps > 0 else '🟢'}\n"
f"💸 <b>Pending Withdrawals:</b> <b>{pending_wds}</b>  {'🔴' if pending_wds > 0 else '🟢'}\n"
f"🔄 <b>Pending Sell Reviews:</b> <b>{pending_sells}</b>  {'🔴' if pending_sells > 0 else '🟢'}\n"
f"<b>▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰</b>",
parse_mode="HTML",
reply_markup=InlineKeyboardMarkup([
...
[InlineKeyboardButton("📥 Pending Deposits", callback_data="ap_pending_deps"),
 InlineKeyboardButton("💸 Withdrawals",      callback_data="ap_pending_wds")],
...
app.add_handler(CallbackQueryHandler(buy_server_menu, pattern="^buy_server_1$"))
app.add_handler(CallbackQueryHandler(buy_server_menu, pattern="^buy_server_2$"))
app.add_handler(CallbackQueryHandler(buy_server_menu, pattern=r"^buy_page_\d+$"))
...
# ── Start health server FIRST so Render sees the open port immediately ───
...
# ── Init DB with retry — bot won't start until DB is reachable ───────────
init_db()
...
for attempt in range(1, 6):
    try:
        ...
        logger.info(f"✅ Webhook deleted (attempt {attempt})")
        break
    except Exception as e:
...
while True:
    try:
        ...
        logger.warning(f"start_polling failed ({e}), retrying in 5s...")
